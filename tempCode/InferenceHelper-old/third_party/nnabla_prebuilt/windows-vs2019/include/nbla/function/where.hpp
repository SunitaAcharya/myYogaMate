...
#ifndef NBLA_FUNCTION_WHERE_HPP
#define NBLA_FUNCTION_WHERE_HPP
...
Inputs:
- N-D array. When condition[i] is true, yield x_true[i], otherwise x_false[i]
for all i.
- N-D array.
- N-D array.
 ...
 */
template <typename T> class Where : public BaseFunction<> {
protected:
public:
  Where(const Context &ctx) : BaseFunction(ctx) {}
  virtual ~Where() {}
