...
/** Randint
 */
#ifndef __NBLA_FUNCTION_RANDINT_HPP__
#define __NBLA_FUNCTION_RANDINT_HPP__
...
public:
  Randint(const Context &ctx, int low, int high, const vector<int> &shape,
          int seed)
      : BaseFunction<int, int, const vector<int> &, int>(ctx, low, high, shape,
                                                         seed),
        low_(low), high_(high), shape_(shape), seed_(seed) {
    NBLA_CHECK(high > low, error_code::value,
               "`high` (%d given) must be larger than `low` (%d given).", high,
               low);
  }
  virtual ~Randint() {}
