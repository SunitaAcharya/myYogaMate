...
#ifndef __NBLA_SOLVER_ADABOUND_HPP__
#define __NBLA_SOLVER_ADABOUND_HPP__
...
*/
template <typename T> class NBLA_API AdaBound : public Solver {
public:
  AdaBound(const Context &ctx, float alpha, float beta1, float beta2, float eps,
           float final_lr, float gamma);
  virtual ~AdaBound();
  virtual string name() { return "AdaBound"; }
